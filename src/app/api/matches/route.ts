...
export async function PUT(req: NextRequest) {
  ...
  // Toggle attended de un participante
  if (toggleAttended) {
...
export async function POST(req: NextRequest) {
  ...
  // Obtener participantes para el resumen
  const registrations = await prisma.registration.findMany({
    where: { eventId, paid: true, attended: true },
    select: { id: true, firstName: true, lastName: true, gender: true, email: true },
  });

  const regMap = new Map(registrations.map((r: any) => [r.id, r]));

  // Calcular cruces mutuos: A seleccionó a B Y B seleccionó a A
  ...
  for (const [personId, selectedIds] of Object.entries(selections)) {
    for (const selectedId of selectedIds) {
      ...
      if (otherSelections.includes(personId)) {
        ...
        if (p1 && p2) {
          matchesFound.push({ person1: p1, person2: p2 });
        }
      }
    }
  }

  // Armar resumen por participante
  const summary: Record<string, { person: typeof registrations[0], matches: typeof registrations[0][] }> = {};

  for (const reg of registrations) {
    summary[reg.id] = { person: reg, matches: [] };
  }
